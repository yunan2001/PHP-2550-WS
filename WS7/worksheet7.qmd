...
ggplot(kidney_df) +
  ...
  ggtitle("Figure 1: Box Plot of gfr by Sex") + 
  theme_minimal()
...
# colnames(coef_df) <- c("Estimate", "Standard Error", "T Statistics", "P Value")
# knitr::kable(coef_df,
#          caption = "Summary of Best Model") %>%
#   kable_styling(latex_options = "HOLD_position")

autoplot(step_model, size=0.5)
```
```{r}
full_model <- glm(sqrt_gfr ~ sbase + dbase + sex + black + log_bascre + log_baseu +
             dbase:age + dbase:sex + dbase:black + dbase:log_bascre + dbase:log_baseu + 
             age:sex + age:black + age:log_bascre + age:log_baseu + 
             sex:black + sex:log_bascre + sex:log_baseu + 
             black:log_bascre + black:log_baseu + log_bascre:log_baseu, 
...
step_model <- step(full_model, direction = "back")
...
```{r}
model_norace <- glm(sqrt_gfr ~ sbase + dbase + age + sex + log_bascre + log_baseu +

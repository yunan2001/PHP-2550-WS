...
# Full model (largest model to be tested)
full_model <- glm(sqrt_gfr ~ (poly(age, degree = 2) + sex + black +
                         sbase + dbase + undetectable_baseu + log_baseu +
                         log_bascre)^2,
                       family = gaussian,
                       data = kidney_df)
...
# Minimal model (smallest model to be tested)
min_model <- glm(sqrt_gfr ~ poly(age, degree = 2) + sex + black + 
                   sbase + dbase + undetectable_baseu + log_baseu + log_bascre,
                 family = gaussian,
                 data = kidney_df)

# Scope bounds for step()
scope_list <- list(
  lower = formula(min_model),  # vars that must remain in the model
  upper = formula(full_model)  # Largest model to consider
  )

# Run step() backwards
step_model_b <- step(full_model,
                     direction = "backward",
                     trace = 0,
                     scope = scope_list)  
...
# Choose best model (If forward and backward agree, done; if they disagree, 
# use LRT to determine best model; see _helpers.R for details)
best_model <- select_best_model(step_model_b, step_model_f)
```

```{r}
kidney_df$undetectable_baseu <- ifelse(kidney_df$baseu <= 0.1, 1, 0)
min_model <- glm(sqrt_gfr ~ sbase + dbase + poly(age, degree = 2) + sex + black + log_bascre + log_baseu + undetectable_baseu, family = gaussian, data = kidney_df)
step(min_model, direction = "forward",
                     trace = 0,
                     k = log(n))
summary(step_min_model)
plot(step_model_b)
```

```{r}
min_model <- glm(sqrt_gfr ~ dbase + age + sex + black + log_bascre + log_baseu, data = kidney_df)
summary(model)
par(mfrow = c(2, 2))
plot(model)


kidney_df$poly_sqrt_gfr1 <- poly(kidney_df$sqrt_gfr, 2)[, 1]
kidney_df$poly_sqrt_gfr2 <- poly(kidney_df$sqrt_gfr, 2)[, 2]

model <- glm(poly_sqrt_gfr1 + poly_sqrt_gfr2 ~ dbase + age + sex + black + log_bascre + log_baseu +
...
model <- glm(log_gfr ~ poly(dbase, 2) + poly(age, 2) + sex + black + log_bascre + log_baseu +
             dbase:age + dbase:sex + dbase:black + dbase:log_bascre + dbase:log_baseu + 
             age:sex + age:black + age:log_bascre + age:log_baseu + 
             sex:black + sex:log_bascre + sex:log_baseu + 
             black:log_bascre + black:log_baseu + log_bascre:log_baseu, 
...
cooks_distances <- cooks.distance(step_model)
plot(cooks_distances, type = "h")
abline(h = 4 / nrow(kidney_df), col = "red")  # Add a threshold line

# Identify high-leverage points
cooks_threshold <- 4 / nrow(kidney_df)

# Identify points above the threshold
influential_points <- which(cooks_distances > cooks_threshold)

# Print out the indices of influential points
influential_points

boxcox_model <- boxcox(glm(sqrt_gfr ~ dbase + age + sex + black + log_bascre + log_baseu, data = kidney_df))
# Find the best lambda (transformation parameter)
best_lambda <- boxcox_model$x[which.max(boxcox_model$y)]

# Apply the Box-Cox transformation
kidney_df$boxcox_gfr <- (kidney_df$sqrt_gfr^best_lambda - 1) / best_lambda

model <- glm(boxcox_gfr ~ dbase + age + sex + black + log_bascre + log_baseu +
             dbase:age + dbase:sex + dbase:black + dbase:log_bascre + dbase:log_baseu + 
             age:sex + age:black + age:log_bascre + age:log_baseu + 
             sex:black + sex:log_bascre + sex:log_baseu + 
             black:log_bascre + black:log_baseu + log_bascre:log_baseu, 
